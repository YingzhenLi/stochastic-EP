...
\end{itemize}
%
For $M=1$ parallel SEP reduces to the original SEP algorithm. For $M=N$ parallel SEP is equivalent to the so-called Averaged EP algorithm proposed in \cite{barthelme:aep} as a theoretical tool to study the convergence properties of normal EP. This work showed that, under fairly restrictive conditions (log-concave and very slow changing likelihood functions\todo[fancyline]{YL: what does slowly varying mean in this context?}), AEP converges to the same fixed points as EP in the large data limit ($N \rightarrow \infty$).
...
\subsection{Distributed SEP: finer grained distributed approximations}
...
\includegraphics[height=5cm]{fig/relationship-algorithms.eps}}
