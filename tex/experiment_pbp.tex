...
%<<<<<<< HEAD
We performed several experiments to assess the accuracy of different implementations of PBP based on ADF, SEP and EP on regression datasets \cite{} following the same experimental protocol as in \cite{hernandez2015probabilistic}.\todo[fancyline]{need to reference the datasets -- they are regression datasets right? Seemed wrong in conclusion} We considered neural networks with 100 hidden units and followed the same experimental protocol 
